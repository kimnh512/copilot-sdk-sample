...
function generateMockTasksFromPrompt(prompt) {
  ...
  if (/(이메일|메일|메일 확인|outlook)/i.test(userText)) {
    commands.push({ name: '이메일 확인', command: 'outlook', type: 'app', priority: 1, order: commands.length + 1, description: '중요한 이메일을 확인하고 회신합니다.' });
  }
  if (/(코드|코드 편집기|VS Code|vscode|visual studio code)/i.test(userText)) {
    commands.push({ name: '코드 편집기 열기', command: 'code', type: 'app', priority: commands.length === 0 ? 1 : 2, order: commands.length + 1, description: '프로젝트를 열어 코드를 확인합니다.' });
  }
  const urlMatch = userText.match(/https?:\/\/[^\s"']+/i);
  if (urlMatch) {
    commands.push({ name: '웹 페이지 열기', command: urlMatch[0], type: 'browser', priority: commands.length === 0 ? 1 : 3, order: commands.length + 1, description: '입력된 URL을 브라우저에서 엽니다.' });
  }
  if (!commands.length) {
    commands.push({ name: '일반 작업', command: 'https://www.google.com/search?q=' + encodeURIComponent(userText), type: 'browser', priority: 3, order: 1, description: userText });
  }
