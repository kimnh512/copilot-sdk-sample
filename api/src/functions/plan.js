...
app.http('plan', {
  ...
  handler: async (request, context) => {
    ...
    if (!text) {
      return { status: 400, jsonBody: { error: 'text 필드가 필요합니다.' } };
    }
    ...
    if (!hasCopilotToken && !isMock) {
      ...
      try {
        ...
        context.log('Copilot raw response:', result);
...
function parseTasksFromText(text) {
  ...
  for (const part of parts) {
    ...
    tasks.push({ name: cleanText, command, type, priority: 3, order: order++, description: cleanText });
  }
  return tasks;
}
...
function fallbackSuggestions(text) {
  ...
  const urlMatch = text.match(/https?:\/\/[^\s]+/i);
  if (urlMatch) {
    suggestions.push({ name: '웹 검색/사이트 열기', command: urlMatch[0], type: 'browser', priority: 2, order: suggestions.length + 1, description: '입력된 링크를 브라우저에서 엽니다.' });
  } else if (/(browse|website|open website|search|google|visit|웹|사이트|검색|구글)/i.test(text)) {
    suggestions.push({ name: '검색 페이지 열기', command: 'https://www.google.com', type: 'browser', priority: 3, order: suggestions.length + 1, description: '웹 검색을 위해 브라우저를 엽니다.' });
  }
  if (/(document|word|doc|write|report|note|memo|문서|기록|노트|메모|작성)/i.test(text)) {
    suggestions.push({ name: '문서 편집기 열기', command: 'notepad', type: 'app', priority: 3, order: suggestions.length + 1, description: '문서 작성 또는 메모를 위해 메모장을 엽니다.' });
  }
  if (/(code|development|editor|vscode|visual studio code|script|코드|코드 편집기)/i.test(text)) {
    suggestions.push({ name: '코드 편집기 열기', command: 'code', type: 'app', priority: 2, order: suggestions.length + 1, description: '코드 편집을 위해 VS Code를 엽니다.' });
  }
  if (/(email|outlook|mail|이메일|메일)/i.test(text)) {
    suggestions.push({ name: '메일 클라이언트 열기', command: 'outlook', type: 'app', priority: 1, order: suggestions.length + 1, description: '이메일을 확인합니다.' });
  }
  if (/(meeting|teams|zoom|video call|call|화상회의|회의|미팅|온라인 미팅)/i.test(text)) {
    suggestions.push({ name: '화상회의 앱 열기', command: 'msteams', type: 'app', priority: 2, order: suggestions.length + 1, description: '화상 회의를 준비합니다.' });
  }
  if (!suggestions.length) {
    suggestions.push({ name: '브라우저 열기', command: 'https://www.google.com', type: 'browser', priority: 4, order: 1, description: '기본 브라우저를 엽니다.' });
  }
  return suggestions;
}
